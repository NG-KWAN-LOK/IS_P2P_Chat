...
export default function Footer({ messageListRef }: FooterProps) {
  ...
  const [isDisplayEmojiPicker, setIsDisplayEmojiPicker] = useState<boolean>(
    false
  );
  const { sendMessage } = useContext(SocketContext);
  useEffect(() => {
    const onClickEsc: any = (e: React.KeyboardEvent<HTMLElement>) => {
      if (e.key == ESC_KEY_CODE) setIsDisplayEmojiPicker(false);
    };
  ...
  return (
    <div className='messages__footer'>
      <div
        className={
          isDisplayEmojiPicker ? "emojiPicker-on" : "emojiPicker-close"
        }
      >
        <Picker
      ...
      <div className='messages__footer__function__bar'>
        ...
        <div className='messages__footer__function__bar__actions'>
          <i
            className={`fa ${isDisplayEmojiPicker ? "fa-close" : "fa-smile-o"}`}
            onClick={() => {
              setIsDisplayEmojiPicker(isDisplayEmojiPicker ? false : true);
            }}
          />
